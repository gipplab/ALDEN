...
class BaseRetriever:
    ...
    def _search(self, query: str, id: str, num: int, return_score: bool, ) -> List[Dict[str, str]]:
    ...
    def _fetch(self, query: str, id: str):
        pass
    ...
    def batch_fetch(self, *args, **kwargs):
        return self._batch_fetch(*args, **kwargs)
...
class DenseRetriever(BaseRetriever):
    ...
    def load_corpus(self, corpus):
        if corpus is None:
            ...
            self.corpus = corpus
        self.id2idx = json.load(open(os.path.join(os.path.split(self.corpus_path)[0], 'id2idx.json')))
        if os.path.exists(os.path.join(os.path.split(self.corpus_path)[0], 'page2idx.json')):
            self.page2idx = json.load(open(os.path.join(os.path.split(self.corpus_path)[0], 'page2idx.json')))
        else:
            self.page2idx = {}
            for k, v in tqdm(self.id2idx.items()):
                temp = {}
                idx = list(range(v[0], v[1]))
                page_number = self.corpus[v[0]: v[1]]['id']
    ...
    def _fetch(self, query: str, id=''):
        for i, q in zip(query, id):
            ...
            if isinstance(corpus_idx, int):
                return self.corpus[corpus_idx]
            else:
                return corpus_idx

    def _search(self, query: str, id: str, num: int = None, return_score=False):
        if num is None:
            num = self.topk
        query_emb = self.encoder.encode([query])
        idx_range = self.id2idx[id]
        sel = faiss.IDSelectorRange(idx_range[0], idx_range[1])
        params = faiss.SearchParameters(sel=sel)
        scores, idxs = self.index.search(query_emb, k=num, params=params)
